...
//  Modficado por Gabriel Edid Harari A01782146 y Paolo Antonio Pires Cano A01749355
...
std::set <std::string> Busqueda<T>::busquedaSecuencialPuertos(std::vector<T> e, const int valor)
{
    ...
    while (i < size) {
        if (e[i].getPuertoDestino() != "-" && std::stoi(e[i].getPuertoDestino()) <= valor) {
            encontrado.insert(e[i].getPuertoDestino());
        }
        ++i;
    }
    return encontrado;
